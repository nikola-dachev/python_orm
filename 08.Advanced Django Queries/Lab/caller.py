...
def product_quantity_ordered():
    total_quantity_ordered = (Product.objects
                              .annotate(ordered_quantity = Sum('orderproduct__quantity'))
                              .exclude(ordered_quantity= None)
                              .order_by('-ordered_quantity'))
    result = []
    for product in total_quantity_ordered:
        result.append(f"Quantity ordered of {product.name}: {product.ordered_quantity}")

    return '\n'.join(result)


def ordered_products_per_customer():
    orders = Order.objects.prefetch_related('products').order_by('pk')
    result = []
    for order in orders:
        result.append(f"Order ID: {order.pk}, Customer: {order.customer.username}")
        all_products_in_current_order = order.products.all()
        for product in all_products_in_current_order:
...
def give_discount():
    query = Q(is_available = True) & Q(price__gt=3.00)
    new_price = F("price") * 0.7

    Product.objects.filter(query).update(price = new_price)
