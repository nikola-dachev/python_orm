...
def find_books_by_genre_and_language(book_genre, book_language):
    searched_books = Book.objects.filter(genre = book_genre, language = book_language)
    return searched_books

def find_authors_nationalities():
    searched_authors = Author.objects.exclude(nationality= None)
    result = [
...
def order_books_by_year():
    searched_books = Book.objects.order_by("publication_year", "title")
    result = [
...
def filter_authors_by_nationalities(nationality):
    searched_authors = Author.objects.filter(nationality = nationality).order_by("first_name", "last_name")
    result = [
        author.biography if author.biography is not None
...
def filter_authors_by_birth_year(first_year, second_year):
    searched_authors =Author.objects.filter(birth_date__year__range=(first_year, second_year)) .order_by("-birth_date")
    result = [
...
def change_reviewer_name(reviewer_name, new_name):
    review = Review.objects.filter(reviewer_name= reviewer_name).update(reviewer_name = new_name)
